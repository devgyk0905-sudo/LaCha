...
const CANDLES = [
  ...
  { id:6, cat:'2봉 패턴', name:'장악형', signal:'neutral', desc:'두 번째 캔들이 첫 번째를 완전히 감싸는 패턴', shape:'2번째 몸통이 1번째 몸통 완전히 포함', signalText:'상승장악: 반등 / 하락장악: 하락', note:'두 캔들 방향이 반대여야 함', keywords:['장악형','인겔핑','engulfing'] },
  { id:7, cat:'3봉 패턴', name:'모닝스타', signal:'up', desc:'하락 추세 바닥에서 나오는 상승 반전 패턴', shape:'①장대음봉 → ②도지/팽이(갭) → ③장대양봉', signalText:'바닥 반전 — 강한 상승 신호', note:'2번 캔들이 도지일수록 신뢰도 높음', keywords:['모닝스타','바닥반전','상승전환'] },
  { id:8, cat:'3봉 패턴', name:'이브닝스타', signal:'down', desc:'상승 추세 고점에서 나오는 하락 반전 패턴', shape:'①장대양봉 → ②도지/팽이 → ③장대음봉', signalText:'고점 반전 — 하락 신호', note:'2번 캔들이 도지일수록 신뢰도 높음', keywords:['이브닝스타','고점반전','하락전환'] },
  { id:9, cat:'3봉 패턴', name:'백삼병', signal:'up', desc:'연속 3개 양봉 — 강한 상승 신호', shape:'연속 3개 양봉 / 각 저점 상승', signalText:'상승 추세 강화', note:'3번째 양봉 저점 이탈 시 무효', keywords:['백삼병','연속양봉','쓰리화이트솔저'] },
  { id:10, cat:'3봉 패턴', name:'흑삼병', signal:'down', desc:'연속 3개 음봉 — 강한 하락 신호', shape:'연속 3개 음봉 / 각 고점 하락', signalText:'하락 추세 강화', note:'1번째 음봉 고점 돌파 시 무효', keywords:['흑삼병','연속음봉','쓰리블랙크로우'] },
  { id:11, cat:'로드트랙 패턴', name:'로드트랙', signal:'neutral', desc:'추후 추가 예정', shape:'—', signalText:'—', note:'수업 내용 업데이트 후 추가', keywords:['로드트랙'] },
]
...
function CandleSVG({ name }) {
  const shapes = {
    ...
    '장악형':   <><rect x="18" y="22" width="14" height="18" fill={RED} rx="1"/><rect x="32" y="12" width="16" height="36" fill={GREEN} rx="1"/></>,
    '모닝스타': <><rect x="6"  y="10" width="12" height="24" fill={RED}   rx="1"/><rect x="28" y="32" width="8"  height="7"  fill={GRAY}  rx="1"/><rect x="46" y="16" width="12" height="20" fill={GREEN} rx="1"/><line x1="12" y1="8"  x2="12" y2="10" stroke={RED}   strokeWidth="1.5"/><line x1="12" y1="34" x2="12" y2="40" stroke={RED}   strokeWidth="1.5"/><line x1="32" y1="30" x2="32" y2="32" stroke={GRAY}  strokeWidth="1.5"/><line x1="32" y1="39" x2="32" y2="44" stroke={GRAY}  strokeWidth="1.5"/><line x1="52" y1="12" x2="52" y2="16" stroke={GREEN} strokeWidth="1.5"/></>,
    '이브닝스타':<><rect x="6"  y="28" width="12" height="24" fill={GREEN} rx="1"/><rect x="28" y="20" width="8"  height="7"  fill={GRAY}  rx="1"/><rect x="46" y="10" width="12" height="24" fill={RED}   rx="1"/><line x1="12" y1="22" x2="12" y2="28" stroke={GREEN} strokeWidth="1.5"/><line x1="32" y1="18" x2="32" y2="20" stroke={GRAY}  strokeWidth="1.5"/><line x1="32" y1="27" x2="32" y2="32" stroke={GRAY}  strokeWidth="1.5"/><line x1="52" y1="34" x2="52" y2="40" stroke={RED}   strokeWidth="1.5"/></>,
    '백삼병':   <><rect x="6"  y="44" width="12" height="18" fill={GREEN} rx="1"/><line x1="12" y1="40" x2="12" y2="44" stroke={GREEN} strokeWidth="1.5"/><rect x="26" y="30" width="12" height="20" fill={GREEN} rx="1"/><line x1="32" y1="26" x2="32" y2="30" stroke={GREEN} strokeWidth="1.5"/><rect x="46" y="14" width="12" height="22" fill={GREEN} rx="1"/><line x1="52" y1="10" x2="52" y2="14" stroke={GREEN} strokeWidth="1.5"/></>,
    '흑삼병':   <><rect x="6"  y="10" width="12" height="18" fill={RED}   rx="1"/><line x1="12" y1="28" x2="12" y2="34" stroke={RED}   strokeWidth="1.5"/><rect x="26" y="22" width="12" height="22" fill={RED}   rx="1"/><line x1="32" y1="44" x2="32" y2="50" stroke={RED}   strokeWidth="1.5"/><rect x="46" y="36" width="12" height="22" fill={RED}   rx="1"/><line x1="52" y1="58" x2="52" y2="64" stroke={RED}   strokeWidth="1.5"/></>,
    '로드트랙': <><line x1="32" y1="10" x2="32" y2="54" stroke={GRAY} strokeWidth="1.5" strokeDasharray="3 2"/><text x="32" y="36" textAnchor="middle" fill={GRAY} fontSize="9">추후</text></>,
  }
  const w = ['모닝스타','이브닝스타','백삼병','흑삼병'].includes(name) ? 72 : 64
  return (
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className={`text-sm ${muted}`}>단일봉 · 2봉 · 3봉 · 로드트랙 패턴 — 클릭하면 상세 보기</p>
      </div>

      <PageSearch query={query} setQuery={setQuery} placeholder="캔들 패턴 검색... (예: 모닝스타, 흑삼병, 도지)" />
